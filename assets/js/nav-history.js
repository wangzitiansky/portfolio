...
export async function loadNavHistory() {
	if (_cache !== null) return getNavHistory();
  try {
    const resp = await fetch('/api/nav');
    if (!resp.ok) throw new Error(`HTTP ${resp.status}`);
    const arr = await resp.json();
		if (!Array.isArray(arr)) throw new Error('服务端返回了无效的净值数据');
		_cache = arr;
		return getNavHistory();
	} catch (e) {
		_cache = null;
		throw new Error(`净值历史加载失败: ${e.message || e}`);
  }
}
...
export async function recordToday(total, todayPnl, todayPnlPct, count) {
	const history = (await loadNavHistory()).map(item => ({ ...item }));
